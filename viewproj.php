<?php
    ...
    $error;
    if(isset($_GET["pname"]) && isset($_GET["uname"])){
        $pname = $_GET["pname"];
        $id = base64_encode($_GET["uname"]);
        $data = $conn->selectMany("SELECT html, css, js from projects where name='$pname' AND id='$id' AND type='public';");
        if($data->num_rows){
...
<body>
    ...
    if(isset($error)){
        echo "<h1 class='error centered-text' style='font-size: 3rem'; margin: 'auto'>$error</h1>";
        echo "<h1 class='error centered-text' style='font-size:5rem'>¯\_(ツ)_/¯</h1>";
    }else{
        echo "<h2>HTML</h2>";
